...
// social media
if ( ! function_exists( 'ecobit_social' ) ) {
	function ecobit_social( $args = array()  ){
		...
		$url = ecobit_opt('ecobit_social_url');
...
function ecobit_contact_form7_shortcode(){

    // contact form list
    $getforms['cs'] = __( 'Custom Shortcode', 'ecobit' );
    // Instruction
    $Instruction = ''; 
    ...
    if( defined('WPCF7_VERSION') ){
        ...
        if( $loop->have_posts() ){
            ...
            $Instruction = __( 'Contact form not found.', 'ecobit' );
        }
    }else{
        $url = admin_url( 'plugins.php' );
        
        $Instruction = sprintf( __( 'If you want to use contact form 7, Please install and active contact form 7 plugin. %s Click here to install %s  ' , 'ecobit' ), '<a target="_blank" href="'.esc_url( $url ).'">', '</a>'  );
    }
...
// Popular post count
function ecobit_get_post_views($postID){
    $count_key = 'ecobit_post_views_count';
...
function ecobit_set_post_views($postID) {
    $count_key = 'ecobit_post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
...
// blog post categoty 
function ecobit_get_post_cat(){
    $cats = get_categories();

    $categories = array( 'na' => esc_html__( 'Select post category', 'ecobit' ) );
