...
/**
 *
 * ecobit elementor about page section widget.
 *
 * @since 1.0
 */
class Ecobit_Contact extends Widget_Base {

    public function get_name() {
        return 'ecobit-contact';
    }

    public function get_title() {
        return __( 'Contact', 'ecobit' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->start_controls_section(
            'contact_info',
            [
                'label' => __( 'Contact Info', 'ecobit' ),
            ]
        );

        $this->add_control(
            'info', [
                'label'         => __( 'Create Contact Info', 'ecobit' ),
                ...
                'fields'  => [
                    [
                        ...
                        'default'     => esc_html__( 'Dhaka, Bangladesh', 'ecobit' )
                    ],
                    [
                        'name'    => 'desc',
                        'label'   => __( 'Contact Descriptions', 'ecobit' ),
                        'type'    => Controls_Manager::TEXTAREA,
                        'default' => esc_html__( 'Write something...', 'ecobit' )
                    ],
                    [
                        'name'  => 'icon',
                        'label' => __( 'Icon', 'ecobit' ),
                        'type'  => Controls_Manager::ICON,
                        'options' => ecobit_themify_icon()
                    ]
        ...
        $this->start_controls_section(
            'contact_form',
            [
                'label' => __( 'Contact Form', 'ecobit' ),
            ]
        );
        $this->add_control(
            'contact_form_title',
            [
                'label'     => esc_html__( 'Contact Form Title', 'ecobit' ),
                'type'      => Controls_Manager::TEXT,
                'label_block' => true,
                'default'   => esc_html__('Get in Touch', 'ecobit')
            ]
        );
        $this->add_control(
            'contact_formshortcode',
            [
                'label'     => esc_html__( 'Form Shortcode', 'ecobit' ),
                'type'      => Controls_Manager::TEXT,
                'label_block' => true
            ]
        );
        ...
        $this->start_controls_section(
            'style_content_color', [
                'label' => __( 'Style Content Color', 'ecobit' ),
        ...
        $this->add_control(
            'color_title', [
                'label'     => __( 'Right Text Title Color', 'ecobit' ),
        ...
        $this->add_control(
            'color_desc', [
                'label'     => __( 'Right Text Sub Title Color', 'ecobit' ),
                'type'      => Controls_Manager::COLOR,
                'selectors' => [
