 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_ecobit_process_simple_like', 'ecobit_process_simple_like' );
add_action( 'wp_ajax_ecobit_process_simple_like', 'ecobit_process_simple_like' );
function ecobit_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'ecobit' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'ecobit' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'ecobit' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'ecobit' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'ecobit' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'ecobit' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'ecobit_show_user_likes' );
add_action( 'edit_user_profile', 'ecobit_show_user_likes' );
function ecobit_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'ecobit' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'ecobit' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
